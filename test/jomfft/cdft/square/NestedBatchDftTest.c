...
#include <jomfft/cdft/square/NestedBatchDft.h>

#include "../BatchDftTest_.h"

static struct NS(ComplexFunction) *CDftSquare_NestedBatchDftFactory_create(
  ...
  return NS(CDftSquare_NestedBatchDft_new)(data, backward, &f, batch_size,
                                           x_stride_0, x_stride_1, y_stride_0,
                                           y_stride_1, inplace, num_threads);
}

void NS(CDftSquare_NestedBatchDftTest)() {
  struct NS(CDft_BatchDftFactory) *factory = NS(CDft_BatchDftFactory_new)(
      CDftSquare_NestedBatchDftFactory_create, NULL, NULL);
