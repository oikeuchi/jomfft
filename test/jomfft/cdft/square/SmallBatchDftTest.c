...
#include <jomfft/cdft/square/SmallBatchDft.h>

#include "../BatchDftTest_.h"

static struct NS(ComplexFunction) *CDftSquare_SmallBatchDftFactory_create(
  ...
  return NS(CDftSquare_SmallBatchDft_new)(data, backward, &f, batch_size,
                                          x_stride_0, x_stride_1, y_stride_0,
                                          y_stride_1, inplace, num_threads);
}

void NS(CDftSquare_SmallBatchDftTest)() {
  struct NS(CDft_BatchDftFactory) *factory = NS(CDft_BatchDftFactory_new)(
      CDftSquare_SmallBatchDftFactory_create, NULL, NULL);
