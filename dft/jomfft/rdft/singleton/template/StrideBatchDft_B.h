...
#define RDFT_LOAD_RI_0(n, z0, zh)                                              \
  do {                                                                         \
    *z0 = RV_LOAD_S((Real *)x, 2 * x_stride_1);                                \
    *zh = RV_LOAD_S((Real *)(x + x_stride_0 * (n) / 2), 2 * x_stride_1);       \
  } while (0)
#define RDFT_LOAD_RI_0_(n) RV_LOAD_S((Real *)x, 2 * x_stride_1)
#define RDFT_STORE(i, z) RV_STORE_S(y + y_stride_0 * (i), y_stride_1, (z))

void FUNCTION(int dft_size, const Complex *w, Long batch_size, Complex *x,
              Long x_stride_0, Long x_stride_1, Real *y, Long y_stride_0,
              Long y_stride_1) {
