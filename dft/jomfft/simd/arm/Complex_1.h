#pragma once

#include "Real_2.h"

#define CV_VECTOR RV_VECTOR

static FORCEINLINE CV_VECTOR CV_LOAD_S(const Complex *p, Long s) {
  (void)s;
  return RV_LOAD((const Real *)p);
}

static FORCEINLINE void CV_STORE_S(Complex *p, Long s, CV_VECTOR x) {
  (void)s;
  RV_STORE((Real *)p, x);
}

#if USE_SINGLE

static FORCEINLINE CV_VECTOR CV_CONJ(CV_VECTOR x) {
  static const uint32x2_t flags = {0, ARM_UINT32_SIGN_FLAG};
  return vreinterpret_f32_u32(veor_u32(vreinterpret_u32_f32(x), flags));
}

static FORCEINLINE CV_VECTOR CV_REVERSE(CV_VECTOR x) { return x; }

static FORCEINLINE void RV_LOAD_RI_S(const Complex *p, Long s, RV_VECTOR *x_r,
                                     RV_VECTOR *x_i) {
  RV_VECTOR t0 = RV_LOAD((const Real *)p);
  RV_VECTOR t1 = RV_LOAD((const Real *)(p + s));
  *x_r = vuzp1_f32(t0, t1);
  *x_i = vuzp2_f32(t0, t1);
}

static FORCEINLINE void RV_STORE_RI_S(Complex *p, Long s, RV_VECTOR x_r,
                                      RV_VECTOR x_i) {
  RV_VECTOR t0 = vzip1_f32(x_r, x_i);
  RV_VECTOR t1 = vzip2_f32(x_r, x_i);
  RV_STORE((Real *)p, t0);
  RV_STORE((Real *)(p + s), t1);
}

#else

static FORCEINLINE CV_VECTOR CV_CONJ(CV_VECTOR x) {
  static const uint64x2_t flags = {0, ARM_UINT64_SIGN_FLAG};
  return vreinterpretq_f64_u64(veorq_u64(vreinterpretq_u64_f64(x), flags));
}

static FORCEINLINE CV_VECTOR CV_REVERSE(CV_VECTOR x) { return x; }

static FORCEINLINE void RV_LOAD_RI_S(const Complex *p, Long s, RV_VECTOR *x_r,
                                     RV_VECTOR *x_i) {
  RV_VECTOR t0 = RV_LOAD((const Real *)p);
  RV_VECTOR t1 = RV_LOAD((const Real *)(p + s));
  *x_r = vuzp1q_f64(t0, t1);
  *x_i = vuzp2q_f64(t0, t1);
}

static FORCEINLINE void RV_STORE_RI_S(Complex *p, Long s, RV_VECTOR x_r,
                                      RV_VECTOR x_i) {
  RV_VECTOR t0 = vzip1q_f64(x_r, x_i);
  RV_VECTOR t1 = vzip2q_f64(x_r, x_i);
  RV_STORE((Real *)p, t0);
  RV_STORE((Real *)(p + s), t1);
}

#endif

#include "Complex.h"
